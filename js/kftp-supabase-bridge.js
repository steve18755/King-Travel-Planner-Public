...
(function(){
  ...
  function sessionToLocal(row){
    ...
    const displayName = profile.display_name || profile.name || [profile.first_name, profile.last_name].filter(Boolean).join(' ') || localId;
  ...
  async function fetchAppUser(){
    ...
    const {data,error}=await db().from('app_users').select('auth_user_id,profile_id,household_id,role,approved').eq('auth_user_id',user.id).maybeSingle();
    if(error) throw error; if(!data) throw new Error('This login exists in Supabase Auth, but it is not linked in app.app_users yet.'); if(!data.approved) throw new Error('This login exists, but it is not approved in app.app_users yet.');
    if(expectedProfileId && data.profile_id!==expectedProfileId){try{await client.auth.signOut();}catch(e){} throw new Error('This account is linked to '+data.profile_id+', not '+expectedProfileId+'.');}
  ...
  function assetCandidates(){
    const base=['assets/images/KingCrest.PNG','assets/images/people/Stephen.png','assets/images/pets/pepper_small.jpg','assets/images/pets/june_small.jpg','assets/images/ui/dashboard_beach_hero.png'];
    const out=new Map();
